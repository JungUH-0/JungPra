...
# FRAME_INTERVAL = 1
# MAX_FRAMES     = 30
# LANDMARK_DIM   = 126  # x,y,z × 21관절 × 2손
...
#         num_hands=2,
#         min_hand_detection_confidence=0.3,  # 낮게 설정 (정적 영상 감지율 향상)
...
#     """프레임 1장에서 양손 관절 x,y,z 좌표 126개 추출"""
#     rgb      = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
#     mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb)
#     result   = detector.detect(mp_image)

#     left  = np.zeros(21 * 3, dtype=np.float32)  # 63개
#     right = np.zeros(21 * 3, dtype=np.float32)  # 63개
...
#     return np.concatenate([left, right]), detected  # 126개
...
#     print(f"   동작 감지 OFF → 손 감지되면 무조건 수집")
...
#         hand_ratio   = len(landmark_seq) / max(total_frames, 1) * 100
#         print(f"    ✅ 손 감지 {len(landmark_seq)}프레임 / 전체 {total_frames}프레임 | 감지율: {hand_ratio:.0f}% → {word_name}.npy")
#         if hand_ratio < 50:
#             print(f"    ⚠️  손 감지율 낮음! 조명이나 손 위치 확인 필요")
#             total_no_hand += 1
...
FRAME_INTERVAL = 1
MAX_FRAMES     = 30
LANDMARK_DIM   = 42   # x,y × 21관절 × 1손 (SignBridge 형식)
...
def extract_landmarks(frame, detector):
    """
    SignBridge 형식: x,y만 × 21관절 = 42개
    양손이 감지되면 오른손 우선, 없으면 왼손 사용
    """
    rgb      = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb)
    result   = detector.detect(mp_image)

    coords   = np.zeros(21 * 2, dtype=np.float32)  # 42개
...
def preprocess_all_videos():
    ...
    print(f"🚀 총 {len(video_files)}개 영상 처리 시작")
    print(f"   SignBridge 형식: x,y × 21관절 = 42차원")
    print(f"   동작 감지 OFF → 손 감지되면 무조건 수집")
    detector = init_detector()
    print(f"   ✅ 모델 로드 완료\n{'='*55}")
