# ============================================================
# PyTorch 3 - torch2와 동일 구조 + AdamW + ReduceLROnPlateau
#
#   이미지        128 x 128 x 3
#   배치          32
#   옵티마이저     AdamW (lr=1e-3, weight_decay=1e-4)
#   스케줄러       ReduceLROnPlateau (factor=0.5, patience=2)
#   epoch         최대 30 (직접 구현한 EarlyStopping, patience=7)
...
training_data = datasets.ImageFolder(root=f"{DATA_DIR}/training", transform=train_transform)
...
hist = {"train_acc": [], "train_loss": [], "val_acc": [], "val_loss": [], "lr": []}
...
for t in range(30):
    ...
    # Keras처럼 epoch 단위 요약을 한 줄로 출력 (학습률 변화도 같이)
    print(
        f"[Epoch {t + 1:>2}]  "
