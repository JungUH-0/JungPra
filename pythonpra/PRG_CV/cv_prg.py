...
class TrafficWeak(QMainWindow):
    def __init__(self):
        ...
        self.signFiles = [['child.png', '어린이'], ['child2.jpg', '어린이보호구역'], ['speed30.png', '속도제한 30']]
        self.signImgs = []
    ...
    def is_valid_quad(self, corners):
        ...
        if area < 2000 or area > 40000:  # 80000 -> 40000으로 더 줄임
            return False
        ...
        if hull_area < area * 0.8:  # 0.7 -> 0.8로 더 엄격하게
            return False

        # 박스 너비/높이가 너무 크면 제거
        if w > 400 or h > 400:  # 400픽셀 이상이면 표지판 아님
            return False
    ...
    def detect_signs(self, frame):
        ...
        for (kp_tmpl, des_tmpl, tmpl, label) in self.template_kp_des:
            ...
            if inliers < 6 or inliers / len(good) < 0.4:
                continue
    ...
    def update_frame(self):
        ...
        if self.frame_count % 3 == 0:
            ...
            if detected:
                ...
                self.detected_timer = self.DETECTED_HOLD
            else:
        ...
        if self.last_detected:
            self.label.setText(f'검출: {", ".join(self.last_detected)} 보호구역입니다. 30km로 서행하세요.')
